...
**בקצרה במספרים:** 2,292 שורות Java ב-39 קבצים, 19 תבניות HTML, ו-71 טסטים אוטומטיים. אפס שורות CSS או JavaScript משלנו — כל העיצוב הוא Bootstrap 5. הקובץ הגדול ביותר הוא כ-150 שורות; אפשר לקרוא את הפרויקט כולו בערב אחד.
...
**1. פורמט WebP במידה הנכונה.** כל תמונה 800×600 (כפול מהמשבצת של 400×300, כדי להישאר חדה במסכי retina) ושוקלת בערך 10KB — 106KB לכל עשר. שליחת JPEG בגודל 2000px למשבצת של 400px היא הטעות הנפוצה ביותר בנושא.
...
- **איך יודעים שהכול עובד?** 71 טסטים אוטומטיים (`./mvnw test`) — סעיף 8, ובנוסף סוויטת בדיקות בדפדפן אמיתי.
...
- **איך דאגתם שהתמונות ייטענו מהר?** WebP במידה הנכונה (‏106KB לכל עשר), ‏`width`/`height` שמונעים תזוזת פריסה (‏CLS=0), ‏`loading="lazy"` בקטלוג, ‏`fetchpriority="high"` בדף המוצר, וכתובות עם חתימת תוכן שמאפשרות מטמון לשנה — סעיף 4.8.
